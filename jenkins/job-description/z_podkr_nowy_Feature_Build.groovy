...
def job_config = [
    job: [
        name: "z_podkr_nowy_Feature_Build",
        agent: "maven"
    ]
]
def config = [
...
def customLib = library identifier: 'z_podkr_nowy_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/z_podkr_nowy_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.ZPodkrNowy.new()
    ...
    stage('Prepare') {
      steps {
        ...
        script {
          ...
          jslGit.checkout( config, "jarro06", "z_podkr_nowy", branch_name)
          }
        }
      }
stage('Build') {
...
"""
def job = pipelineJob("z_podkr_nowy_Feature_Build");
...
job.with {
    ...
    authenticationToken('y3ox161JfFdgX570iyYIkziNjdUKQtkj')
